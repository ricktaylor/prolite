...
static const term_t* skip_debug_info(const term_t* p, const debug_info_t** debug_info)
{
	if (debug_info)
...
{
	assert(get_term_type(b) == prolite_atom);
...
{
	assert(get_term_type(b) == prolite_compound);

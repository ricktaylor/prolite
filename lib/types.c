...
term_t* emit_buffer_append(emit_buffer_t* out, size_t count)
{
	...
	{
		size_t new_count = out->m_alloc * 2;
		if (out->m_count + count > new_count)
			new_count = out->m_count + count;
...
static const term_t* emit_debug_info(emit_buffer_t* out, const debug_info_t* debug_info)
{
	// TODO
	return (const term_t*)1;
}
...
void pack_string(term_t out[2], const unsigned char* str, size_t len)
{
	...
	return emit_string_type(out,prolite_atom,str,len,builtin);
}
...
const term_t* emit_string(emit_buffer_t* out, prolite_type_t type, const unsigned char* str, size_t len, int external, const debug_info_t* debug_info)
{
	...
	if (r && debug_info && !emit_debug_info(out,debug_info))
		r = NULL;
...
const term_t* emit_predicate(emit_buffer_t* out, size_t arity, const unsigned char* functor, size_t functor_len, int external, const debug_info_t* debug_info)
{
	...
	if (r)
	{
		...
		if (arity <= MAX_ARITY_EMBED && functor_len <= 5)
		{
			...
			if (debug_info && !emit_debug_info(out,debug_info))
				r = NULL;
		}
		else if (arity <= MAX_ARITY_BUILTIN && (builtin = is_builtin_string(functor,functor_len)) != MAX_BUILTIN_ATOM)
		{
			...
			r->m_u64val = (PACK_TYPE(type) | PACK_MANT_48(UINT64_C(0x4000) | ((arity & MAX_ARITY_BUILTIN) << 32) | builtin));
...
static void copy_term_inner(context_t* context, emit_buffer_t* out, const term_t* src, int shallow, uint64_t** var_mapping, size_t* var_count, jmp_buf* jmp)
{
	uint64_t all48;
	prolite_type_t type;
	int have_debug_info;
	...
	case prolite_var:
		{
			...
			if (v != src)
				copy_term_inner(context,out,v,shallow,var_mapping,var_count,jmp);
			else
			{
	...
	case prolite_compound:
		{
			...
			for (; arity--; n = get_next_arg(n))
				copy_term_inner(context,out,n,shallow,var_mapping,var_count,jmp);
		}
		break;
...
term_t* copy_term(context_t* context, emit_buffer_t* out, const term_t* src, int shallow, size_t* var_count)
{
	...
	if (!setjmp(jmp))
	{
		...
		copy_term_inner(context,out,src,shallow,&var_mapping,&vc,&jmp);

		if (var_count)
			*var_count = vc;
	}
	else
		out->m_buf = allocator_free(out->m_a,out->m_buf);

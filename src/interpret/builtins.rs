...
fn solve_unify_with_occurs_check(frame: Frame, args: &[usize], next: &mut dyn Solver) -> Response {
    if frame.occurs_check(args[0], args[1]) {
        Response::Fail
    } else {
...
const BUILTINS: phf::Map<&'static str, Builtin> = phf_map! {
    ...
    "current_char_conversion/2" => Builtin::Solve(flags::solve_current_char_conversion),
